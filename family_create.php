...
// Initial State
$targetDocType = $_POST['target_doc_type'] ?? 'financial';
$instructions   = trim((string)($_POST['instructions'] ?? 'Draft a comprehensive FDR Position Statement for Financial Remedy proceedings under Section 25 Matrimonial Causes Act 1973. Seek a 60/40 capital division in favour of the Applicant to meet primary housing needs of the minor children and a 45% pension sharing order.'));
$opponentText   = trim((string)($_POST['opponent_text'] ?? 'Extract from Respondent Form E: \"The Respondent contends that the matrimonial assets should be divided strictly 50/50 and that his pension acquired prior to marriage should be ring-fenced. The Respondent proposes an immediate sale of the family home with equal distribution of net equity.\"'));
$factsText      = trim((string)($_POST['facts'] ?? 'The parties married on 18 June 2011 and separated on 20 October 2023 (12-year marriage). There are two minor children of the family aged 9 and 6 residing primarily with the Applicant mother. Matrimonial home is valued at £650,000 with a mortgage of £200,000 (net equity £450,000). The Respondent earns £95,000 p.a. and has pension assets of £420,000 CEV. The Applicant earns £22,000 p.a. part-time and has pension assets of £35,000 CEV. The Applicant\'s mortgage capacity is limited to £120,000.'));
$evidenceText   = trim((string)($_POST['evidence_available'] ?? '• Marriage Certificate dated 18 June 2011
• Applicant Form E with comprehensive bank statements and payslips
• Joint Single Expert Property Valuation (£650,000)
• Actuarial Pension Sharing Report (PAG compliant)
• Mortgage capacity assessment report (£120,000)'));
$remedyText     = trim((string)($_POST['remedy_requested'] ?? 'Transfer of family home to Applicant with 60% equity share (£270,000), 45% Pension Sharing Order against Respondent’s pension, and spousal maintenance of £800/month for a term of 5 years.'));

$party1         = trim((string)($_POST['applicant_name'] ?? 'Hannah Charlotte Davies'));
$party2         = trim((string)($_POST['respondent_name'] ?? 'Marcus Robert Davies'));
$matterRef      = trim((string)($_POST['case_reference'] ?? ('AEP-FAM-' . date('Y') . '-' . rand(100, 999))));
...
$outputTitle    = $_POST['output_title'] ?? 'AI Family Law & Matrimonial Intelligence Workspace DRAFT';
...
// Load sample handler
if ($action === 'sample') {
    $instructions = 'Draft a comprehensive FDR Position Statement for Financial Remedy proceedings under Section 25 Matrimonial Causes Act 1973. Seek a 60/40 capital division in favour of the Applicant to meet primary housing needs of the minor children and a 45% pension sharing order.';
    $opponentText = 'Extract from Respondent Form E: \"The Respondent contends that the matrimonial assets should be divided strictly 50/50 and that his pension acquired prior to marriage should be ring-fenced. The Respondent proposes an immediate sale of the family home with equal distribution of net equity.\"';
    $factsText = 'The parties married on 18 June 2011 and separated on 20 October 2023 (12-year marriage). There are two minor children of the family aged 9 and 6 residing primarily with the Applicant mother. Matrimonial home is valued at £650,000 with a mortgage of £200,000 (net equity £450,000). The Respondent earns £95,000 p.a. and has pension assets of £420,000 CEV. The Applicant earns £22,000 p.a. part-time and has pension assets of £35,000 CEV. The Applicant\'s mortgage capacity is limited to £120,000.';
    $evidenceText = '• Marriage Certificate dated 18 June 2011
• Applicant Form E with comprehensive bank statements and payslips
• Joint Single Expert Property Valuation (£650,000)
• Actuarial Pension Sharing Report (PAG compliant)
• Mortgage capacity assessment report (£120,000)';
    $remedyText = 'Transfer of family home to Applicant with 60% equity share (£270,000), 45% Pension Sharing Order against Respondent’s pension, and spousal maintenance of £800/month for a term of 5 years.';
    $party1 = 'Hannah Charlotte Davies';
    $party2 = 'Marcus Robert Davies';
...
// Execute Analysis
$analysis = analyseFamilyCase($caseData);
...
if (in_array($action, ['generate', 'draft', 'generate_draft', 'analyse'])) {
    $doc = generateFamilyDoc($analysis, $caseData, ['reference' => $matterRef], $targetDocType);
...
if ($action === 'save_to_matter' && !empty($outputDraft)) {
    try {
        ...
        if ($mId <= 0) {
            ...
            $stmtM->execute([
                $matterRef,
                $party1 . ' vs ' . $party2 . ' (' . ucfirst('cases') . ')',
                $cId,
                $party1,
...
$pageTitle = "AI Family Law & Matrimonial Intelligence Workspace — AEP Legal Intelligence Platform";
?>
<!doctype html>
<html lang="en">
<head>
...
<div class="topbar">
  <div class="brand">⚖️ AEP Legal Intelligence <span class="badge-law">FAMILY LAW PRACTICE GROUP</span></div>
  <nav>
...
<div class="hero">
  <h1>👨‍👩‍👧 👨‍👩‍👧 Family Law & Matrimonial Finance Intelligence</h1>
  <p>AI-Powered Financial Remedy (S.25 MCA 1973), Child Arrangements (S.8 CA 1989), FDR Position Statements & Non-Molestation Orders</p>
</div>
      ...
      <div>
          ...
          <div class="panel-body">
            <div class="form-group">
              ...
              <select name="target_doc_type" class="form-control" onchange="document.getElementById('mainForm').submit();">
                    <option value="financial" <?= $targetDocType === 'financial' ? 'selected' : '' ?>>⚖️ Section 25 MCA 1973 FDR Position Statement</option>
                    <option value="child" <?= $targetDocType === 'child' ? 'selected' : '' ?>>🧸 Child Arrangements Application (Section 8 Children Act 1989)</option>
                    <option value="non_mol" <?= $targetDocType === 'non_mol' ? 'selected' : '' ?>>🛡️ Non-Molestation & Occupation Order Statement (Part IV FLA 1996)</option>
                    <option value="settlement" <?= $targetDocType === 'settlement' ? 'selected' : '' ?>>🤝 Without Prejudice Calderbank Financial Settlement Proposal</option>
                    <option value="advice" <?= $targetDocType === 'advice' ? 'selected' : '' ?>>📋 Comprehensive Matrimonial Advice & Housing Needs Assessment</option>
        ...
        <div class="panel-card">
          ...
          <div class="panel-body">
            ...
            <div class="grid-2">
              <div class="form-group">
                <label>Applicant / Client Name:</label>
                <input type="text" name="applicant_name" class="form-control" value="<?= htmlspecialchars($party1) ?>" />
              </div>
              <div class="form-group">
                <label>Respondent Name:</label>
                <input type="text" name="respondent_name" class="form-control" value="<?= htmlspecialchars($party2) ?>" />
              </div>
            </div>
          </div>
        </div>
