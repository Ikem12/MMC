...
<div class="header">
    ...
    <div style="font-size: 11pt; font-weight: bold; margin-top: 4px;">Employment & Tribunal Intelligence</div>
</div>

<div class="meta">
    <p><strong>DOCUMENT TITLE:</strong> <?= htmlspecialchars($record['claim_type'] ?? 'Legal Record') ?><br>
    <strong>REFERENCE:</strong> <?= htmlspecialchars($record['case_reference'] ?? 'N/A') ?><br>
    <strong>PARTY / CLIENT:</strong> <?= htmlspecialchars($record['claimant_name'] ?? 'N/A') ?><br>
    <strong>DATE:</strong> <?= date('d F Y', strtotime($record['created_at'] ?? 'now')) ?></p>
</div>

<div class="content"><?= htmlspecialchars($record['claim_details'] ?? '') ?></div>
