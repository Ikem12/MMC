...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_id'])) {
    ...
    $pdo->prepare("DELETE FROM skeleton_arguments WHERE id = ?")->execute([(int)$_POST['delete_id']]);
    header('Location: skeleton_list.php');
    exit;
}

$search = trim($_GET['search'] ?? '');
if ($search) {
    $stmt = $pdo->prepare("SELECT * FROM skeleton_arguments WHERE case_title LIKE ? OR case_number LIKE ? OR court LIKE ? OR party LIKE ? ORDER BY created_at DESC");
    $stmt->execute(["%$search%", "%$search%", "%$search%", "%$search%"]);
} else {
    $stmt = $pdo->query("SELECT * FROM skeleton_arguments ORDER BY created_at DESC");
}
...
<head>
<meta charset="utf-8"/>
<title>Skeleton Arguments — AEP</title>
<style>
...
.stats{display:grid;grid-template-columns:repeat(3,1fr);gap:16px;margin-bottom:24*
