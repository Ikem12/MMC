...
// Initial State
$targetDocType = $_POST['target_doc_type'] ?? 'defence_statement';
$instructions   = trim((string)($_POST['instructions'] ?? 'Draft a Section 5 CPIA 1996 Defence Case Statement for charges of Section 20 Grievous Bodily Harm. Assert self-defence under s.76 CJPA 2008 and request specific disclosure of unused body-worn video and 999 audio logs under Section 8 CPIA.'));
$opponentText   = trim((string)($_POST['opponent_text'] ?? 'Extract from CPS Summary (MG5): \"The Crown alleges that on 12 August 2024 at 23:15 outside the Red Lion Pub, the Defendant unlawfully and maliciously assaulted the complainant with a pint glass, causing a 3cm laceration. Crown relies on witness statement of bar manager and CCTV footage.\"'));
$factsText      = trim((string)($_POST['facts'] ?? 'Defendant was at the venue with his partner. The complainant, who was heavily intoxicated, approached the Defendant, uttered racial abuse, and threw a punch towards the Defendant. The Defendant reacted instinctively in self-defence to ward off the violent attack, causing the glass to shatter. Defendant immediately remained at the scene and cooperated with police. Defendant has no previous convictions (good character).'));
$evidenceText   = trim((string)($_POST['evidence_available'] ?? '• Attending Officer Body-Worn Video transcripts
• Custody Record showing Defendant\'s physical injuries
• Independent witness statement confirming complainant threw first punch
• Medical toxicology report of complainant showing acute intoxication
• Character references from employer and community leaders'));
$remedyText     = trim((string)($_POST['remedy_requested'] ?? 'Formal Acquittal / Discontinuance under CPS Code for Crown Prosecutors; immediate grant of unconditional bail pending trial.'));

$party1         = trim((string)($_POST['defendant_name'] ?? 'Tariq Tariqson'));
$party2         = trim((string)($_POST['prosecution'] ?? 'Crown Prosecution Service (CPS) / Snaresbrook Crown Court'));
$matterRef      = trim((string)($_POST['case_reference'] ?? ('AEP-CRM-' . date('Y') . '-' . rand(100, 999))));
...
$outputTitle    = $_POST['output_title'] ?? 'AI Criminal Defence & Evidence Intelligence Workspace DRAFT';
...
// Load sample handler
if ($action === 'sample') {
    $instructions = 'Draft a Section 5 CPIA 1996 Defence Case Statement for charges of Section 20 Grievous Bodily Harm. Assert self-defence under s.76 CJPA 2008 and request specific disclosure of unused body-worn video and 999 audio logs under Section 8 CPIA.';
    $opponentText = 'Extract from CPS Summary (MG5): \"The Crown alleges that on 12 August 2024 at 23:15 outside the Red Lion Pub, the Defendant unlawfully and maliciously assaulted the complainant with a pint glass, causing a 3cm laceration. Crown relies on witness statement of bar manager and CCTV footage.\"';
    $factsText = 'Defendant was at the venue with his partner. The complainant, who was heavily intoxicated, approached the Defendant, uttered racial abuse, and threw a punch towards the Defendant. The Defendant reacted instinctively in self-defence to ward off the violent attack, causing the glass to shatter. Defendant immediately remained at the scene and cooperated with police. Defendant has no previous convictions (good character).';
    $evidenceText = '• Attending Officer Body-Worn Video transcripts
• Custody Record showing Defendant\'s physical injuries
• Independent witness statement confirming complainant threw first punch
• Medical toxicology report of complainant showing acute intoxication
• Character references from employer and community leaders';
    $remedyText = 'Formal Acquittal / Discontinuance under CPS Code for Crown Prosecutors; immediate grant of unconditional bail pending trial.';
    $party1 = 'Tariq Tariqson';
    $party2 = 'Crown Prosecution Service (CPS) / Snaresbrook Crown Court';
...
// Execute Analysis
$analysis = analyseCriminalCase($caseData);
...
if (in_array($action, ['generate', 'draft', 'generate_draft', 'analyse'])) {
    $doc = generateCriminalDoc($analysis, $caseData, ['reference' => $matterRef], $targetDocType);
...
if ($action === 'save_to_matter' && !empty($outputDraft)) {
    try {
        ...
        if ($mId <= 0) {
            ...
            $stmtM->execute([
                $matterRef,
                $party1 . ' vs ' . $party2 . ' (' . ucfirst('criminal_cases') . ')',
                $cId,
                $party1,
...
$pageTitle = "AI Criminal Defence & Evidence Intelligence Workspace — AEP Legal Intelligence Platform";
?>
<!doctype html>
<html lang="en">
<head>
...
<div class="topbar">
  <div class="brand">⚖️ AEP Legal Intelligence <span class="badge-law">CRIMINAL DEFENCE GROUP</span></div>
  <nav>
...
<div class="hero">
  <h1>⚖️ ⚖️ Criminal Defence & Evidence Intelligence</h1>
  <p>AI-Powered Defence Case Statements (S.5 CPIA 1996), PACE 1984 S.78 Exclusion Applications, Bail Packages & CPS Code Merits</p>
</div>
          ...
          <div class="panel-body">
            <div class="form-group">
              ...
              <select name="target_doc_type" class="form-control" onchange="document.getElementById('mainForm').submit();">
                    <option value="defence_statement" <?= $targetDocType === 'defence_statement' ? 'selected' : '' ?>>⚖️ Defence Case Statement (Section 5 CPIA 1996)</option>
                    <option value="bail" <?= $targetDocType === 'bail' ? 'selected' : '' ?>>🔓 Crown Court / Magistrates Bail Application Skeleton</option>
                    <option value="pace78" <?= $targetDocType === 'pace78' ? 'selected' : '' ?>>🛡️ Section 78 PACE 1984 Application to Exclude Unfair Evidence</option>
                    <option value="mitigation" <?= $targetDocType === 'mitigation' ? 'selected' : '' ?>>📜 Plea in Mitigation & Sentencing Submissions</option>
                    <option value="advice" <?= $targetDocType === 'advice' ? 'selected' : '' ?>>📋 Pre-Trial Evidence Review & Galbraith Merits Advice</option>
          ...
          <div class="panel-body">
            ...
            <div class="grid-2">
              <div class="form-group">
                ...
                <input type="text" name="defendant_name" class="form-control" value="<?= htmlspecialchars($party1) ?>" />
              </div>
              <div class="form-group">
                <label>Prosecuting Authority / Court:</label>
                <input type="text" name="prosecution" class="form-control" value="<?= htmlspecialchars($party2) ?>" />
              </div>
            </div>
          </div>
        </div>
