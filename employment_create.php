...
// Initial State
$targetDocType = $_POST['target_doc_type'] ?? 'et1';
$instructions   = trim((string)($_POST['instructions'] ?? 'Draft a formal ET1 Particulars of Claim for constructive and unfair dismissal under s.98 ERA 1996 and sex discrimination under Equality Act 2010. Include failure to follow the ACAS Code of Practice and claim 25% statutory uplift.'));
$opponentText   = trim((string)($_POST['opponent_text'] ?? 'Extract from Respondent Disciplinary Outcome Letter: \"We reject your grievance regarding bonus allocation. We find that the restructuring of your sales territory was an operational business decision. Your resignation on 14 July 2024 is treated as voluntary without notice entitlement.\"'));
$factsText      = trim((string)($_POST['facts'] ?? 'Claimant (Senior Account Executive) was employed from 1 March 2021 to 14 July 2024 (over 3 years continuous service). Claimant maintained exemplary performance ratings. Following return from maternity leave in January 2024, Claimant was stripped of her key accounts without consultation. Grievances submitted on 10 April and 12 May were dismissed without formal investigation. Claimant resigned in response to fundamental breach of mutual trust and confidence on 14 July 2024.'));
$evidenceText   = trim((string)($_POST['evidence_available'] ?? '• Contract of Employment dated 1 March 2021
• Performance appraisals (Exceeding Targets 2021-2023)
• Email removing key accounts dated 15 January 2024
• Written grievances dated 10 April and 12 May 2024
• ACAS Early Conciliation Certificate R184920/24/01'));
$remedyText     = trim((string)($_POST['remedy_requested'] ?? 'Full Basic Award (£2,800.00), Compensatory Award for 6 months net wage loss (£24,500.00), Middle Vento Band for Injury to Feelings (£16,000.00), and 25% ACAS statutory uplift.'));

$party1         = trim((string)($_POST['claimant_name'] ?? 'Eleanor Vance (Senior Account Executive)'));
$party2         = trim((string)($_POST['respondent_name'] ?? 'Meridian Tech Global UK Ltd'));
$matterRef      = trim((string)($_POST['case_reference'] ?? ('AEP-EMP-' . date('Y') . '-' . rand(100, 999))));
...
$outputTitle    = $_POST['output_title'] ?? 'AI Employment Law & Tribunal Workspace DRAFT';
...
// Load sample handler
if ($action === 'sample') {
    $instructions = 'Draft a formal ET1 Particulars of Claim for constructive and unfair dismissal under s.98 ERA 1996 and sex discrimination under Equality Act 2010. Include failure to follow the ACAS Code of Practice and claim 25% statutory uplift.';
    $opponentText = 'Extract from Respondent Disciplinary Outcome Letter: \"We reject your grievance regarding bonus allocation. We find that the restructuring of your sales territory was an operational business decision. Your resignation on 14 July 2024 is treated as voluntary without notice entitlement.\"';
    $factsText = 'Claimant (Senior Account Executive) was employed from 1 March 2021 to 14 July 2024 (over 3 years continuous service). Claimant maintained exemplary performance ratings. Following return from maternity leave in January 2024, Claimant was stripped of her key accounts without consultation. Grievances submitted on 10 April and 12 May were dismissed without formal investigation. Claimant resigned in response to fundamental breach of mutual trust and confidence on 14 July 2024.';
    $evidenceText = '• Contract of Employment dated 1 March 2021
• Performance appraisals (Exceeding Targets 2021-2023)
• Email removing key accounts dated 15 January 2024
• Written grievances dated 10 April and 12 May 2024
• ACAS Early Conciliation Certificate R184920/24/01';
    $remedyText = 'Full Basic Award (£2,800.00), Compensatory Award for 6 months net wage loss (£24,500.00), Middle Vento Band for Injury to Feelings (£16,000.00), and 25% ACAS statutory uplift.';
    $party1 = 'Eleanor Vance (Senior Account Executive)';
    $party2 = 'Meridian Tech Global UK Ltd';
...
// Execute Analysis
$analysis = analyseEmploymentCase($caseData);
...
if (in_array($action, ['generate', 'draft', 'generate_draft', 'analyse'])) {
    $doc = generateEmploymentDoc($analysis, $caseData, ['reference' => $matterRef], $targetDocType);
...
if ($action === 'save_to_matter' && !empty($outputDraft)) {
    try {
        ...
        if ($mId <= 0) {
            ...
            $stmtM->execute([
                $matterRef,
                $party1 . ' vs ' . $party2 . ' (' . ucfirst('employment_cases') . ')',
                $cId,
                $party1,
...
$pageTitle = "AI Employment Law & Tribunal Workspace — AEP Legal Intelligence Platform";
?>
<!doctype html>
<html lang="en">
<head>
...
<div class="topbar">
  <div class="brand">⚖️ AEP Legal Intelligence <span class="badge-law">EMPLOYMENT PRACTICE GROUP</span></div>
  <nav>
...
<div class="hero">
  <h1>👔 👔 Employment Tribunal & Workplace Intelligence</h1>
  <p>AI-Powered ET1/ET3 Drafting, Unfair Dismissal & Discrimination Analysis, ACAS Conciliation & Schedule of Loss</p>
</div>
      ...
      <div>
          ...
          <div class="panel-body">
            <div class="form-group">
              ...
              <select name="target_doc_type" class="form-control" onchange="document.getElementById('mainForm').submit();">
                    <option value="et1" <?= $targetDocType === 'et1' ? 'selected' : '' ?>>⚖️ ET1 Particulars of Claim (Unfair Dismissal & Discrimination)</option>
                    <option value="et3" <?= $targetDocType === 'et3' ? 'selected' : '' ?>>🛡️ ET3 Grounds of Resistance / Employer Defence</option>
                    <option value="schedule" <?= $targetDocType === 'schedule' ? 'selected' : '' ?>>📊 Comprehensive Schedule of Loss & Compensation</option>
                    <option value="without_prejudice" <?= $targetDocType === 'without_prejudice' ? 'selected' : '' ?>>🤝 Without Prejudice / Settlement Agreement Proposal</option>
                    <option value="witness" <?= $targetDocType === 'witness' ? 'selected' : '' ?>>📜 Claimant / Respondent Witness Statement</option>
                    <option value="grievance" <?= $targetDocType === 'grievance' ? 'selected' : '' ?>>📋 Formal Written Workplace Grievance</option>
        ...
        <div class="panel-card">
          ...
          <div class="panel-body">
            ...
            <div class="grid-2">
              <div class="form-group">
                <label>Claimant Name & Job Title:</label>
                <input type="text" name="claimant_name" class="form-control" value="<?= htmlspecialchars($party1) ?>" />
              </div>
              <div class="form-group">
                <label>Respondent Employer:</label>
