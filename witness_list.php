...
if ($search) {
    $stmt = $pdo->prepare("SELECT * FROM witness_statements WHERE case_title LIKE ? OR witness_name LIKE ? OR case_number LIKE ? ORDER BY created_at DESC");
    $stmt->execute(["%$search%", "%$search%", "%$search%"]);
} else {
    $stmt = $pdo->query("SELECT * FROM witness_statements ORDER BY created_at DESC");
}
...
<head>
<meta charset="utf-8"/>
<title>Witness Statements — AEP</title>
<style>
