...
if ($search) {
    $stmt = $pdo->prepare("SELECT * FROM grounds_of_appeal WHERE case_title LIKE ? OR case_number LIKE ? OR lower_court LIKE ? OR appeal_court LIKE ? ORDER BY created_at DESC");
    $stmt->execute(["%$search%", "%$search%", "%$search%", "%$search%"]);
} else {
    $stmt = $pdo->query("SELECT * FROM grounds_of_appeal ORDER BY created_at DESC");
}
...
<div class="topbar">
  ...
  <div style="display:flex;gap:20px">
    ...
    <a href="appeal_create.php">🏛️ New Appeal</a>
...
<div class="container">
  ...
  <!-- STATS -->
  <?php
    $total = $pdo->query("SELECT COUNT(*) FROM grounds_of_appeal")->fetchColumn();
    $draft = $pdo->query("SELECT COUNT(*) FROM grounds_of_appeal WHERE status='draft'")->fetchColumn();
    $filed = $pdo->query("SELECT COUNT(*) FROM grounds_of_appeal WHERE status='filed'")->fetchColumn();
  ?>
  <div class="stats">
    <div class="stat-card">
    ...
    <div class="stat-card">
      <div class="stat-number"><?php echo $filed; ?></div>
      <div class="stat-label">Filed in Court</div>
    </div>
  </div>

  <div class="page-header">
    <h2>🏛️ Grounds of Appeal</h2>
    <a href="appeal_create.php" class="btn btn-primary">➕ New Grounds of Appeal</a>
  </div>

  <!-- SEARCH -->
  <form method="GET" class="search-bar">
    <input type="text" name="search" value="<?php echo htmlspecialchars($search); ?>"
           placeholder="Search by case title, case number or court..."/>
    <button type="submit">🔍 Search</button>
    <?php if ($search): ?>
      <a href="appeal_list.php" class="btn btn-warning">✖ Clear</a>
    <?php endif; ?>
  </form>
  ...
  <div class="card">
    <?php if (empty($records)): ?>
      <div class="empty">
        ...
        <p>No grounds of appeal found.</p>
        <a href="appeal_create.php" class="btn btn-primary" style="margin-top:12px">➕ Create First Appeal</a>
      </div>
    <?php else: ?>
      <table>
        <thead>
        ...
        <tbody>
          ...
          <tr>
            <td><?php echo $i + 1; ?></td>
            <td><strong><?php echo htmlspecialchars($r['case_title']); ?></strong></td>
            <td><?php echo htmlspecialchars($r['case_number'] ?: '—'); ?></td>
            <td><?php echo htmlspecialchars($r['lower_court'] ?: '—'); ?></td>
            <td><?php echo htmlspecialchars($r['appeal_court'] ?: '—'); ?></td>
            <td><?php echo htmlspecialchars($r['party'] ?: '—'); ?></td>
            <td><?php echo $r['judgment_date'] ? date('d M Y', strtotime($r['judgment_date'])) : '—'; ?></td>
            <td>
