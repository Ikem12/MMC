...
if (isset($_GET['delete'])) {
    ...
    if ($delId > 0) {
        $stmt = $pdo->prepare("DELETE FROM grounds_of_appeal WHERE id = ?");
        $stmt->execute([$delId]);
        header("Location: appeal_list.php?msg=deleted");
        exit;
    }
}
...
if ($search !== '') {
    $where[] = "(case_number LIKE ? OR party LIKE ? OR case_title LIKE ?)";
    $params[] = "%{$search}%";
    $params[] = "%{$search}%";
    $params[] = "%{$search}%";
}
...
$sql = "SELECT * FROM grounds_of_appeal WHERE " . implode(' AND ', $where) . " ORDER BY id DESC";
...
try {
    $dSql = "SELECT d.*, m.reference as matter_reference, m.title as matter_title 
             ...
             WHERE m.practice_area LIKE '%Appellate Advocacy%' OR m.practice_area LIKE '%Appellate Advocacy & Grounds of Appeal%'
...
<head>
    ...
    <title>Appellate Advocacy & Grounds of Appeal — AEP Legal Platform</title>
...
<div class="hero">
    <div class="hero-content">
        <div>
            <h1>📜 Appellate Advocacy & Grounds of Appeal</h1>
            <p>Form N161 Grounds of Appeal, Permission to Appeal Skeleton Arguments (CPR Part 52) & Appellate Review</p>
        </div>
        <div style="display: flex; gap: 10px;">
            <a href="appeal_create.php" class="btn btn-success" style="font-size: 14px; padding: 11px 22px;">⚡ Open AI Workspace / New Draft</a>
        </div>
    </div>
</div>
...
<div class="container">
    ...
    <div class="toolbar">
        <form method="GET" class="search-form">
            ...
            <?php if ($search || $status): ?>
                <a href="appeal_list.php" class="btn btn-outline">Clear</a>
            <?php endif; ?>
        </form>
        <div>
            <a href="appeal_create.php" class="btn btn-primary">+ New Record</a>
        </div>
    </div>
    ...
    <div class="card">
        <div class="card-header">
            <span>📋 Appellate Advocacy & Grounds of Appeal Records</span>
        ...
        <?php if (empty($records)): ?>
            <div class="empty-state">
                ...
                <a href="appeal_create.php" class="btn btn-primary" style="margin-top: 14px;">⚡ Open Appellate Advocacy & Grounds of Appeal Workspace</a>
            </div>
        <?php else: ?>
                        ...
                        <?php foreach ($records as $r): ?>
                            <tr>
                                <td style="font-weight: 700; color: #1e3a8a;">
                                    <a href="appeal_view.php?id=<?= $r['id'] ?>" style="color: #1e3a8a; text-decoration: none;">
                                        <?= htmlspecialchars($r['case_number'] ?? ('REF-' . $r['id'])) ?>
                                    </a>
                                </td>
                                <td style="font-weight: 600;">
                                    <?= htmlspecialchars($r['party'] ?? 'N/A') ?>
                                </td>
                                <td>
                                    <?= htmlspecialchars($r['case_title'] ?? 'Case Assessment') ?>
                                </td>
                                ...
                                <td style="text-align: right;">
                                    <div style="display: inline-flex; gap: 6px;">
                                        <a href="appeal_view.php?id=<?= $r['id'] ?>" class="btn btn-outline" style="font-size: 11px; padding: 5px 10px;">👁️ View</a>
                                        <a href="appeal_print.php?id=<?= $r['id'] ?>" class="btn btn-outline" style="font-size: 11px; padding: 5px 10px;" target="_blank">🖨 Print</a>
                                        <a href="appeal_create.php?id=<?= $r['id'] ?>" class="btn btn-outline" style="font-size: 11px; padding: 5px 10px;">⚡ AI Workspace</a>
                                        <a href="appeal_list.php?delete=<?= $r['id'] ?>" class="btn btn-danger" style="font-size: 11px; padding: 5px 10px;" onclick="return confirm('Delete this record?');">🗑 Delete</a>
                                    </div>
                                </td>
                            </tr>
                        <?php endforeach; ?>
