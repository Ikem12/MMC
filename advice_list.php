...
if (isset($_GET['delete'])) {
    ...
    if ($delId > 0) {
        $stmt = $pdo->prepare("DELETE FROM legal_advice WHERE id = ?");
        $stmt->execute([$delId]);
        header("Location: advice_list.php?msg=deleted");
        exit;
    }
}
...
if ($search !== '') {
    $where[] = "(file_no LIKE ? OR client_name LIKE ? OR subject LIKE ?)";
    $params[] = "%{$search}%";
    $params[] = "%{$search}%";
    $params[] = "%{$search}%";
}
...
$sql = "SELECT * FROM legal_advice WHERE " . implode(' AND ', $where) . " ORDER BY id DESC";
...
try {
    $dSql = "SELECT d.*, m.reference as matter_reference, m.title as matter_title 
             ...
             WHERE m.practice_area LIKE '%Legal Advice%' OR m.practice_area LIKE '%Legal Advice & Merits Assessment%'
...
<head>
    ...
    <title>Legal Advice & Merits Assessment — AEP Legal Platform</title>
...
<div class="hero">
    <div class="hero-content">
        <div>
            <h1>💡 Legal Advice & Merits Assessment</h1>
            <p>Formal Legal Opinions, Merits Prospects Rating (95%+), Risk Evaluation & Step-by-Step Strategic Advice</p>
        </div>
        <div style="display: flex; gap: 10px;">
            <a href="advice_create.php" class="btn btn-success" style="font-size: 14px; padding: 11px 22px;">⚡ Open AI Workspace / New Draft</a>
        </div>
    </div>
</div>
...
<div class="container">
    ...
    <div class="toolbar">
        <form method="GET" class="search-form">
            ...
            <?php if ($search || $status): ?>
                <a href="advice_list.php" class="btn btn-outline">Clear</a>
            <?php endif; ?>
        </form>
        <div>
            <a href="advice_create.php" class="btn btn-primary">+ New Record</a>
        </div>
    </div>
    ...
    <div class="card">
        <div class="card-header">
            <span>📋 Legal Advice & Merits Assessment Records</span>
        ...
        <?php if (empty($records)): ?>
            <div class="empty-state">
                ...
                <a href="advice_create.php" class="btn btn-primary" style="margin-top: 14px;">⚡ Open Legal Advice & Merits Assessment Workspace</a>
            </div>
        <?php else: ?>
                        ...
                        <?php foreach ($records as $r): ?>
                            <tr>
                                <td style="font-weight: 700; color: #1e3a8a;">
                                    <a href="advice_view.php?id=<?= $r['id'] ?>" style="color: #1e3a8a; text-decoration: none;">
                                        <?= htmlspecialchars($r['file_no'] ?? ('REF-' . $r['id'])) ?>
                                    </a>
                                </td>
                                <td style="font-weight: 600;">
                                    <?= htmlspecialchars($r['client_name'] ?? 'N/A') ?>
                                </td>
                                <td>
                                    <?= htmlspecialchars($r['subject'] ?? 'Case Assessment') ?>
                                </td>
                                ...
                                <td style="text-align: right;">
                                    <div style="display: inline-flex; gap: 6px;">
                                        <a href="advice_view.php?id=<?= $r['id'] ?>" class="btn btn-outline" style="font-size: 11px; padding: 5px 10px;">👁️ View</a>
                                        <a href="advice_print.php?id=<?= $r['id'] ?>" class="btn btn-outline" style="font-size: 11px; padding: 5px 10px;" target="_blank">🖨 Print</a>
                                        <a href="advice_create.php?id=<?= $r['id'] ?>" class="btn btn-outline" style="font-size: 11px; padding: 5px 10px;">⚡ AI Workspace</a>
                                        <a href="advice_list.php?delete=<?= $r['id'] ?>" class="btn btn-danger" style="font-size: 11px; padding: 5px 10px;" onclick="return confirm('Delete this record?');">🗑 Delete</a>
                                    </div>
                                </td>
                            </tr>
                        <?php endforeach; ?>
