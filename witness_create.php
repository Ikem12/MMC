...
// Initial State
$targetDocType = $_POST['target_doc_type'] ?? 'witness_statement';
$instructions   = trim((string)($_POST['instructions'] ?? 'Draft a trial witness statement in the first person strictly complying with CPR PD 57AC. Focus on direct personal knowledge of contract negotiations on 12 January 2024 and receipt of delivery records without arguing law.'));
$opponentText   = trim((string)($_POST['opponent_text'] ?? 'Extract from Opposing Witness Statement: \"I attended the meeting on 12 January 2024. It was explicitly agreed verbally that delivery times were of the essence and that any 1-hour delay would forfeit all right to payment.\"'));
$factsText      = trim((string)($_POST['facts'] ?? 'The witness (Marcus Vance) is the Managing Director of the Claimant company. He conducted the negotiation meeting on 12 January 2024 alongside commercial director Sarah Jenkins. At no point was any verbal clause agreed making delivery time of the essence. The witness personally supervised all 18 freight consignments between May and August 2024 and inspected clean CMR delivery notes signed by the Defendant\'s warehouse manager.'));
$evidenceText   = trim((string)($_POST['evidence_available'] ?? '• Contemporaneous Handwritten Meeting Notes of 12 January 2024
...
$remedyText     = trim((string)($_POST['remedy_requested'] ?? 'Admissibility and full weight attached to witness testimony at trial under CPR Part 32.'));

$party1         = trim((string)($_POST['client_name'] ?? 'Marcus Vance (Managing Director)'));
$party2         = trim((string)($_POST['opposing_party'] ?? 'Claimant (TransGlobal Freight Services Ltd)'));
$matterRef      = trim((string)($_POST['case_reference'] ?? ('AEP-WIT-' . date('Y') . '-' . rand(100, 999))));
...
$outputTitle    = $_POST['output_title'] ?? 'AI Witness Statement Intelligence Workspace DRAFT';
...
// Load sample handler
if ($action === 'sample') {
    $instructions = 'Draft a trial witness statement in the first person strictly complying with CPR PD 57AC. Focus on direct personal knowledge of contract negotiations on 12 January 2024 and receipt of delivery records without arguing law.';
    $opponentText = 'Extract from Opposing Witness Statement: \"I attended the meeting on 12 January 2024. It was explicitly agreed verbally that delivery times were of the essence and that any 1-hour delay would forfeit all right to payment.\"';
    $factsText = 'The witness (Marcus Vance) is the Managing Director of the Claimant company. He conducted the negotiation meeting on 12 January 2024 alongside commercial director Sarah Jenkins. At no point was any verbal clause agreed making delivery time of the essence. The witness personally supervised all 18 freight consignments between May and August 2024 and inspected clean CMR delivery notes signed by the Defendant\'s warehouse manager.';
    $evidenceText = '• Contemporaneous Handwritten Meeting Notes of 12 January 2024
• Email correspondence confirming agenda and executed contract terms
• 18 Clean Signed CMR Consignment Delivery Receipts
• Telephone attendance notes from credit control follow-ups';
    $remedyText = 'Admissibility and full weight attached to witness testimony at trial under CPR Part 32.';
    $party1 = 'Marcus Vance (Managing Director)';
    $party2 = 'Claimant (TransGlobal Freight Services Ltd)';
...
// Execute Analysis
$analysis = analyseWitnessCase($caseData);
...
if (in_array($action, ['generate', 'draft', 'generate_draft', 'analyse'])) {
    $doc = generateWitnessDoc($analysis, $caseData, ['reference' => $matterRef], $targetDocType);
...
if ($outputDraft === '' && $action !== 'reset') {
    $doc = generateWitnessDoc($analysis, $caseData, ['reference' => $matterRef], $targetDocType);
    $outputDraft = $doc['content'] ?? ($doc['draft'] ?? ($doc['Generated Draft']['content'] ?? ''));
    $outputTitle = $doc['title'] ?? 'AI Witness Statement Intelligence Workspace DRAFT';
}
...
if ($action === 'save_matter') {
    try {
        $mId = $selectedMatter;
        if ($mId === 0) {
            $stmt = $pdo->prepare("INSERT INTO matters (reference, title, client_name, practice_area, status, created_at, updated_at) VALUES (?, ?, ?, 'Witness Statement Intelligence', 'open', datetime('now'), datetime('now'))");
            $stmt->execute([$matterRef, 'Witness Statement Intelligence: ' . ($party1 ?: 'Dispute'), $party1]);
        ...
        $dStmt->execute([$mId, $outputTitle ?: 'AI Witness Statement Intelligence Draft', $outputDraft, $_SESSION['user_id'] ?? 1]);
...
<head>
    <meta charset="UTF-8">
    <title>AI Witness Statement Intelligence Workspace — AEP Legal Intelligence Platform</title>
...
<header class="workspace-header">
    <div class="workspace-title">
        <h1>✍️ Witness Statement Intelligence Workspace</h1>
        <p>AI-Powered Witness Statement Drafting strictly compliant with CPR Practice Direction 57AC, Fact Extraction, Memory Chronology & Statements of Truth</p>
    </div>
    <nav class="top-nav">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 14px;">
                            ...
                            <div class="form-group">
                                <label>Party Calling Witness:</label>
...
function exportWord() {
    ...
    a.href = url;
    a.download = 'AEP-WIT_Draft_' + new Date().toISOString().slice(0,10) + '.doc';
