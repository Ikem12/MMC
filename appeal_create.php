...
// Initial State
$targetDocType = $_POST['target_doc_type'] ?? 'grounds_of_appeal';
$instructions   = trim((string)($_POST['instructions'] ?? 'Draft Form N161 Grounds of Appeal and Skeleton Argument to the Court of Appeal under CPR Part 52. Demonstrate that the trial judge misdirected themselves on the law of contractual interpretation and made findings of fact unsupported by evidence.'));
$opponentText   = trim((string)($_POST['opponent_text'] ?? 'Extract from Respondent Notice: \"The learned Judge\'s decision was an unassailable exercise of judicial discretion on primary facts. The appeal has no real prospect of success and permission should be refused as totally without merit.\"'));
$factsText      = trim((string)($_POST['facts'] ?? 'Judgment was delivered on 10 January 2026 in the High Court (Chancery Division) dismissing the Appellant\'s claim. The learned Judge held that Clause 12 of the Master Haulage Agreement operated as a strict condition precedent barring claim recovery. In doing so, the Judge disregarded the commercial matrix of fact and relied on an alleged verbal waiver that was never pleaded or evidenced at trial.'));
$evidenceText   = trim((string)($_POST['evidence_available'] ?? '• Sealed High Court Trial Order and Approved Approved Judgment Transcript
...
$remedyText     = trim((string)($_POST['remedy_requested'] ?? 'Order granting Permission to Appeal under CPR 52.6; Order setting aside the dismissal and entering judgment for the Appellant with costs.'));

$party1         = trim((string)($_POST['client_name'] ?? 'Apex Global Holdings Ltd (Appellant)'));
$party2         = trim((string)($_POST['opposing_party'] ?? 'TransContinental Logistics Ltd (Respondent)'));
$matterRef      = trim((string)($_POST['case_reference'] ?? ('AEP-APP-' . date('Y') . '-' . rand(100, 999))));
...
$outputTitle    = $_POST['output_title'] ?? 'AI Appellate Advocacy & Grounds of Appeal Workspace DRAFT';
...
// Load sample handler
if ($action === 'sample') {
    $instructions = 'Draft Form N161 Grounds of Appeal and Skeleton Argument to the Court of Appeal under CPR Part 52. Demonstrate that the trial judge misdirected themselves on the law of contractual interpretation and made findings of fact unsupported by evidence.';
    $opponentText = 'Extract from Respondent Notice: \"The learned Judge\'s decision was an unassailable exercise of judicial discretion on primary facts. The appeal has no real prospect of success and permission should be refused as totally without merit.\"';
    $factsText = 'Judgment was delivered on 10 January 2026 in the High Court (Chancery Division) dismissing the Appellant\'s claim. The learned Judge held that Clause 12 of the Master Haulage Agreement operated as a strict condition precedent barring claim recovery. In doing so, the Judge disregarded the commercial matrix of fact and relied on an alleged verbal waiver that was never pleaded or evidenced at trial.';
    $evidenceText = '• Sealed High Court Trial Order and Approved Approved Judgment Transcript
• Executed Commercial Haulage Agreement dated 12 January 2024
• Trial Witness Statements and contemporaneous email correspondence
• Transcripts of Trial Day 3 cross-examination confirming no verbal waiver';
    $remedyText = 'Order granting Permission to Appeal under CPR 52.6; Order setting aside the dismissal and entering judgment for the Appellant with costs.';
    $party1 = 'Apex Global Holdings Ltd (Appellant)';
    $party2 = 'TransContinental Logistics Ltd (Respondent)';
...
// Execute Analysis
$analysis = analyseAppealCase($caseData);
...
if (in_array($action, ['generate', 'draft', 'generate_draft', 'analyse'])) {
    $doc = generateAppealDoc($analysis, $caseData, ['reference' => $matterRef], $targetDocType);
...
if ($outputDraft === '' && $action !== 'reset') {
    $doc = generateAppealDoc($analysis, $caseData, ['reference' => $matterRef], $targetDocType);
    $outputDraft = $doc['content'] ?? ($doc['draft'] ?? ($doc['Generated Draft']['content'] ?? ''));
    $outputTitle = $doc['title'] ?? 'AI Appellate Advocacy & Grounds of Appeal Workspace DRAFT';
}
...
if ($action === 'save_matter') {
    try {
        $mId = $selectedMatter;
        if ($mId === 0) {
            $stmt = $pdo->prepare("INSERT INTO matters (reference, title, client_name, practice_area, status, created_at, updated_at) VALUES (?, ?, ?, 'Appellate Advocacy & Grounds of Appeal', 'open', datetime('now'), datetime('now'))");
            $stmt->execute([$matterRef, 'Appellate Advocacy & Grounds of Appeal: ' . ($party1 ?: 'Dispute'), $party1]);
        ...
        $dStmt->execute([$mId, $outputTitle ?: 'AI Appellate Advocacy & Grounds of Appeal Draft', $outputDraft, $_SESSION['user_id'] ?? 1]);
...
<head>
    <meta charset="UTF-8">
    <title>AI Appellate Advocacy & Grounds of Appeal Workspace — AEP Legal Intelligence Platform</title>
...
<header class="workspace-header">
    <div class="workspace-title">
        <h1>📜 Appellate Advocacy & Grounds of Appeal Workspace</h1>
        <p>AI-Powered Form N161 Grounds of Appeal, Permission to Appeal Skeleton Arguments (CPR Part 52), Judicial Review Appeals & Error of Law Extraction</p>
    </div>
    <nav class="top-nav">
...
function exportWord() {
    ...
    a.href = url;
    a.download = 'AEP-APP_Draft_' + new Date().toISOString().slice(0,10) + '.doc';
