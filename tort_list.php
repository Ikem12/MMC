...
$params = [];
if ($search) {
    $sql .= " AND (claimant_name LIKE ? OR case_reference LIKE ? OR defendant_name LIKE ? OR case_type LIKE ?)";
...
th{background:#6a0572;color:#fff;padding:12px 16px;text-align:left;font-size:0.82rem}
...
<div class="container">
  <div class="toolbar">
    <form method="GET" style="display:flex;gap:12px;flex:1;flex-wrap:wrap">
      <input type="text" name="search" placeholder="Search by claimant, reference, defendant, type..." value="<?php echo htmlspecialchars($search); ?>"/>
      <select name="status">
  ...
  <?php if (empty($cases)): ?>
    <div class="empty">&#9888;&#65039; No tort cases found. <a href="tort_create.php">Create the first one!</a></div>
  <?php else: ?>
  <table>
    <thead><tr><th>#</th><th>Case Reference</th><th>Case Type</th><th>Claimant</th><th>Defendant</th><th>Incident Date</th><th>Damages Claimed</th><th>Status</th><th>Actions</th></tr></thead>
    <tbody>
    <?php foreach($cases as $c): ?>
      <tr>
        ...
        <td><strong><?php echo htmlspecialchars($c['claimant_name'] ?: 'N/A'); ?></strong></td>
        <td><?php echo htmlspecialchars($c['defendant_name'] ?: 'N/A'); ?></td>
        <td><?php echo $c['incident_date'] ? date('d/m/Y', strtotime($c['incident_date'])) : 'N/A'; ?></td>
        <td><?php echo htmlspecialchars($c['damages_claimed'] ?: '—'); ?></td>
        ...
        <td class="action-links">
          <a href="tort_view.php?id=<?php echo $c['id']; ?>" class="link-view">&#128065; View</a>
          <a href="tort_print.php?id=<?php echo $c['id']; ?>" class="link-print">&#128424; Print</a>
        </td>
      </tr>
    <?php endforeach; ?>
    </tbody>
