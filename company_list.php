...
if (isset($_GET['delete'])) {
    ...
    if ($delId > 0) {
        $stmt = $pdo->prepare("DELETE FROM company_cases WHERE id = ?");
        $stmt->execute([$delId]);
...
if ($search !== '') {
    $where[] = "(case_reference LIKE ? OR petitioner_name LIKE ? OR company_name LIKE ?)";
    $params[] = "%{$search}%";
    $params[] = "%{$search}%";
    $params[] = "%{$search}%";
}
...
$sql = "SELECT * FROM company_cases WHERE " . implode(' AND ', $where) . " ORDER BY id DESC";
...
try {
    $dSql = "SELECT d.*, m.reference as matter_reference, m.title as matter_title 
             ...
             WHERE m.practice_area LIKE '%Corporate Law%' OR m.practice_area LIKE '%Corporate Law & Shareholder Intelligence%'
...
<head>
    ...
    <title>Corporate Law & Shareholder Intelligence — AEP Legal Platform</title>
...
<div class="hero">
    <div class="hero-content">
        <div>
            <h1>🏢 Corporate Law & Shareholder Intelligence</h1>
            <p>Section 994 Unfair Prejudice Petitions, Directors Duties Enforcement, Shareholder Buyout Remedies & Articles of Association</p>
        </div>
        <div style="display: flex; gap: 10px;">
            <a href="company_create.php" class="btn btn-success" style="font-size: 14px; padding: 11px 22px;">⚡ Open AI Workspace / New Draft</a>
        </div>
    </div>
</div>
...
<div class="container">
    ...
    <div class="toolbar">
        <form method="GET" class="search-form">
            ...
            <?php if ($search || $status): ?>
                <a href="company_list.php" class="btn btn-outline">Clear</a>
            <?php endif; ?>
        </form>
        <div>
            <a href="company_create.php" class="btn btn-primary">+ New Record</a>
        </div>
    </div>
    ...
    <div class="card">
        <div class="card-header">
            <span>📋 Corporate Law & Shareholder Intelligence Records</span>
        ...
        <?php if (empty($records)): ?>
            <div class="empty-state">
                ...
                <a href="company_create.php" class="btn btn-primary" style="margin-top: 14px;">⚡ Open Corporate Law & Shareholder Intelligence Workspace</a>
            </div>
        <?php else: ?>
                        ...
                        <?php foreach ($records as $r): ?>
                            <tr>
                                <td style="font-weight: 700; color: #1e3a8a;">
                                    <a href="company_view.php?id=<?= $r['id'] ?>" style="color: #1e3a8a; text-decoration: none;">
                                        <?= htmlspecialchars($r['case_reference'] ?? ('REF-' . $r['id'])) ?>
                                    </a>
                                </td>
                                <td style="font-weight: 600;">
                                    <?= htmlspecialchars($r['petitioner_name'] ?? 'N/A') ?>
                                </td>
                                <td>
                                    <?= htmlspecialchars($r['company_name'] ?? 'Case Assessment') ?>
                                </td>
                                ...
                                <td style="text-align: right;">
                                    <div style="display: inline-flex; gap: 6px;">
                                        <a href="company_view.php?id=<?= $r['id'] ?>" class="btn btn-outline" style="font-size: 11px; padding: 5px 10px;">👁️ View</a>
                                        <a href="company_print.php?id=<?= $r['id'] ?>" class="btn btn-outline" style="font-size: 11px; padding: 5px 10px;" target="_blank">🖨 Print</a>
                                        <a href="company_create.php?id=<?= $r['id'] ?>" class="btn btn-outline" style="font-size: 11px; padding: 5px 10px;">⚡ AI Workspace</a>
                                        <a href="company_list.php?delete=<?= $r['id'] ?>" class="btn btn-danger" style="font-size: 11px; padding: 5px 10px;" onclick="return confirm('Delete this record?');">🗑 Delete</a>
                                    </div>
                                </td>
                            </tr>
                        <?php endforeach; ?>
