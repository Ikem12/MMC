...
// Initial State
$targetDocType = $_POST['target_doc_type'] ?? 'psc_dispute';
$instructions   = trim((string)($_POST['instructions'] ?? 'Draft a Formal Notice of Dispute and Cash Call Default under Clause 8 of the Joint Operating Agreement (JOA). Demand immediate cure of unpaid AFEs ($4,200,000) for offshore exploration well and notify forfeiture of participating interest.'));
$opponentText   = trim((string)($_POST['opponent_text'] ?? 'Extract from Non-Operator Notice: \"We dispute the validity of Cash Call No. 14. We maintain that the operator incurred costs in excess of the approved 10% AFE overrun threshold without calling an Operating Committee meeting. We are withholding payment pending an independent joint audit.\"'));
$factsText      = trim((string)($_POST['facts'] ?? 'The Operator and Non-Operator entered into a Joint Operating Agreement on 4 May 2019 governing Offshore Production Licence P.2410 (Block 21/14b). The Operating Committee unanimously approved AFE-2023-09 for deepwater exploration drilling with a budget of $35,000,000. Due to severe seabed geotechnical conditions, emergency safety operations required casing adjustments costing an additional $4,200,000. Under Clause 8.3 of the JOA, the Operator is authorized to incur emergency expenditures to protect life and asset integrity. Non-Operator has failed to remit its 35% participating interest share ($1,470,000) despite formal cash call notices.'));
$evidenceText   = trim((string)($_POST['evidence_available'] ?? '• Executed Joint Operating Agreement (AIPN Model Form) dated 4 May 2019
• Approved AFE-2023-09 and OPCO Meeting Minutes
• Emergency Wellsite Daily Drilling Logs and Well Integrity Engineering Reports
• Cash Call Invoices CC-14 and CC-15 with proof of dispatch
• 30-day formal cure warnings dated 10 June and 15 July 2024'));
$remedyText     = trim((string)($_POST['remedy_requested'] ?? 'Immediate remittance of $1,470,000 plus default interest at LIBOR/SOFR + 4%; forfeiture and pro-rata redistribution of 35% participating interest upon non-cure.'));

$party1         = trim((string)($_POST['client_name'] ?? 'Apex Deepwater Energy Ltd (Operator)'));
$party2         = trim((string)($_POST['opposing_party'] ?? 'PetroGlobal Ventures B.V. (35% Non-Operator)'));
$matterRef      = trim((string)($_POST['case_reference'] ?? ('AEP-ENG-' . date('Y') . '-' . rand(100, 999))));
...
$outputTitle    = $_POST['output_title'] ?? 'AI Energy, Oil & Gas Intelligence Workspace DRAFT';
...
// Load sample handler
if ($action === 'sample') {
    $instructions = 'Draft a Formal Notice of Dispute and Cash Call Default under Clause 8 of the Joint Operating Agreement (JOA). Demand immediate cure of unpaid AFEs ($4,200,000) for offshore exploration well and notify forfeiture of participating interest.';
    $opponentText = 'Extract from Non-Operator Notice: \"We dispute the validity of Cash Call No. 14. We maintain that the operator incurred costs in excess of the approved 10% AFE overrun threshold without calling an Operating Committee meeting. We are withholding payment pending an independent joint audit.\"';
    $factsText = 'The Operator and Non-Operator entered into a Joint Operating Agreement on 4 May 2019 governing Offshore Production Licence P.2410 (Block 21/14b). The Operating Committee unanimously approved AFE-2023-09 for deepwater exploration drilling with a budget of $35,000,000. Due to severe seabed geotechnical conditions, emergency safety operations required casing adjustments costing an additional $4,200,000. Under Clause 8.3 of the JOA, the Operator is authorized to incur emergency expenditures to protect life and asset integrity. Non-Operator has failed to remit its 35% participating interest share ($1,470,000) despite formal cash call notices.';
    $evidenceText = '• Executed Joint Operating Agreement (AIPN Model Form) dated 4 May 2019
• Approved AFE-2023-09 and OPCO Meeting Minutes
• Emergency Wellsite Daily Drilling Logs and Well Integrity Engineering Reports
• Cash Call Invoices CC-14 and CC-15 with proof of dispatch
• 30-day formal cure warnings dated 10 June and 15 July 2024';
    $remedyText = 'Immediate remittance of $1,470,000 plus default interest at LIBOR/SOFR + 4%; forfeiture and pro-rata redistribution of 35% participating interest upon non-cure.';
    $party1 = 'Apex Deepwater Energy Ltd (Operator)';
    $party2 = 'PetroGlobal Ventures B.V. (35% Non-Operator)';
...
// Execute Analysis
$analysis = analyseOilGasCase($caseData);
...
if (in_array($action, ['generate', 'draft', 'generate_draft', 'analyse'])) {
    $doc = generateOilGasDoc($analysis, $caseData, ['reference' => $matterRef], $targetDocType);
...
if ($action === 'save_to_matter' && !empty($outputDraft)) {
    try {
        ...
        if ($mId <= 0) {
            ...
            $stmtM->execute([
                $matterRef,
                $party1 . ' vs ' . $party2 . ' (' . ucfirst('oil_gas_cases') . ')',
                $cId,
                $party1,
...
$pageTitle = "AI Energy, Oil & Gas Intelligence Workspace — AEP Legal Intelligence Platform";
?>
<!doctype html>
<html lang="en">
<head>
...
<div class="topbar">
  <div class="brand">⚖️ AEP Legal Intelligence <span class="badge-law">ENERGY & NATURAL RESOURCES GROUP</span></div>
  <nav>
...
<div class="hero">
  <h1>⚡ ⚡ Energy, Oil & Gas Intelligence Workspace</h1>
  <p>AI-Powered JOA Dispute Notices, Petroleum Licencing Merits, Decommissioning Risk (S.29 Petroleum Act 1998) & Energy Arbitration</p>
</div>
          ...
          <div class="panel-body">
            <div class="form-group">
              ...
              <select name="target_doc_type" class="form-control" onchange="document.getElementById('mainForm').submit();">
                    <option value="psc_dispute" <?= $targetDocType === 'psc_dispute' ? 'selected' : '' ?>>⚖️ Formal Notice of Dispute & JOA Cash Call Default</option>
                    <option value="decom_advice" <?= $targetDocType === 'decom_advice' ? 'selected' : '' ?>>🛡️ Section 29 Petroleum Act 1998 Decommissioning Liability Advice</option>
                    <option value="arbitration" <?= $targetDocType === 'arbitration' ? 'selected' : '' ?>>📜 LCIA / ICC Energy Arbitration Request for Relief</option>
                    <option value="regulatory" <?= $targetDocType === 'regulatory' ? 'selected' : '' ?>>📋 NSTA Regulatory Compliance & Field Development Submission</option>
