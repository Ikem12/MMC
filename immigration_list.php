...
$status = $_GET['status'] ?? '';
$sql = "SELECT * FROM immigration_cases WHERE 1=1";
$params = [];
if ($search) { $sql .= " AND (client_name LIKE ? OR case_reference LIKE ? OR client_nationality LIKE ?)"; $params = array_merge($params,["%$search%","%$search%","%$search%"]); }
...
<head>
<meta charset="utf-8"/>
<title>Immigration Cases &mdash; AEP Legal Platform</title>
<style>
...
.badge-appeal{background:#fde8d8;color:#e67e22}
...
<div class="topbar">
  ...
  <div><a href="dashboard.php">&#127968; Dashboard</a><a href="immigration_create.php">+ New Case</a><a href="logout.php">&#128682; Logout</a></div>
</div>
<div class="hero"><h1>&#9992;&#65039; Immigration Law Cases</h1></div>
<div class="container">
  <div class="toolbar">
    <form method="GET" style="display:flex;gap:12px;flex:1;flex-wrap:wrap">
      <input type="text" name="search" placeholder="Search by name, reference, nationality..." value="<?php echo htmlspecialchars($search); ?>"/>
      <select name="status"><option value="">All Statuses</option><?php foreach(['draft','active','appeal','tribunal','won','lost','withdrawn','closed'] as $s): ?><option value="<?php echo $s;?>" <?php echo $status===$s?'selected':'';?>><?php echo ucfirst($s);?></option><?php endforeach;?></select>
      <button type="submit" class="btn btn-primary">&#128269; Search</button>
      <a href="immigration_list.php" class="btn btn-primary">&#10006; Clear</a>
    </form>
    <a href="immigration_create.php" class="btn btn-blue">+ New Immigration Case</a>
  </div>
  <?php if(empty($cases)): ?><div class="empty">&#9992;&#65039; No immigration cases found. <a href="immigration_create.php">Create the first one!</a></div>
  <?php else: ?>
  <table>
    <thead><tr><th>#</th><th>Case Reference</th><th>Client</th><th>Nationality</th><th>Case Type</th><th>Visa Expiry</th><th>Status</th><th>Actions</th></tr></thead>
    <tbody>
