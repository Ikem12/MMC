...
$cases = $pdo->query("SELECT * FROM tort_cases ORDER BY created_at DESC")->fetchAll(PDO::FETCH_ASSOC);
?>
<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8"/>
  <title>Tort Law — AEP Legal Platform</title>
  <meta name="viewport" content="width=device-width,initial-scale=1"/>
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    body{font-family:Arial,Helvetica,sans-serif;background:#f4f6f9;color:#222}
    header{background:#6a0572;color:#fff;padding:24px 20px;text-align:center}
    header h1{font-size:1.7rem;margin-bottom:4px}
    header p{font-size:0.9rem;opacity:0.85}
    .file-tag{font-size:11px;background:#fff;color:#6a0572;padding:3px 8px;border-radius:4px;margin-left:10px;vertical-align:middle}
    nav{background:#4a0050;padding:10px 20px;display:flex;flex-wrap:wrap;gap:8px;justify-content:center}
    ...
    .top-bar h2{font-size:1.2rem;color:#4a0050}
    .btn{display:inline-block;padding:8px 18px;background:#6a0572;color:#fff;border-radius:4px;text-decoration:none;font-size:0.9rem}
    .btn:hover{background:#4a0050}
    ...
    thead{background:#6a0572;color:#fff}
...
<header>
  <h1>⚠️ Tort Law <span class="file-tag">FILE: tort.php</span></h1>
  <p>Negligence, nuisance, defamation and other civil wrongs</p>
</header>

<nav>
  <a href="index.php">🏠 Home</a>
  <a href="tort.php">Tort Law</a>
  <a href="tort_create.php">New Case</a>
  <a href="tort_checklist.php">Checklist</a>
...
<div class="container">
  ...
  <div class="top-bar">
    <h2>All Tort Law Cases</h2>
    <div style="display:flex;gap:10px;flex-wrap:wrap">
      <a href="tort_checklist.php" class="checklist-btn">📋 Checklist</a>
      <a href="tort_create.php" class="btn btn-green">+ New Case</a>
    </div>
  </div>

  <?php if (empty($cases)): ?>
    <div class="empty">
      <p style="font-size:1.1rem;margin-bottom:12px">No tort law cases yet.</p>
      <a href="tort_create.php" class="btn btn-green">+ Create First Case</a>
    </div>
  <?php else: ?>
    <table>
      <thead>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td><?php echo htmlspecialchars($c['tort_type'] ?? '—'); ?></td>
          <td><?php echo htmlspecialchars($c['claimant'] ?? '—'); ?></td>
          <td><?php echo htmlspecialchars($c['defendant'] ?? '—'); ?></td>
          ...
          <td><a href="tort_view.php?id=<?php echo $c['id']; ?>" class="btn" style="padding:4px 10px;font-size:0.82rem">View</a></td>
        </tr>
        <?php endforeach; ?>
      </tbody>
    </table>
